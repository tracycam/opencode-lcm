...
### Fixed
- Plugin entry module no longer exports `createStoreHooks`: OpenCode's plugin loader invokes every exported function as a plugin factory, so the exported helper was called with a `PluginInput` as its `store` argument, producing broken hooks that logged `store.captureDeferred is not a function` on every captured event; the helper moved to an internal `store-hooks` module (still importable by tests)
